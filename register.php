	...
	if($uname AND $name AND $pwd AND $pwd1){
		if (!preg_match("/^[a-zA-Z ]*$/",$name)) {
			echo "Only letters and white space allowed"; 
				...
				$checkUserSql = "SELECT uname FROM user WHERE uname = '$uname'";
				$checkResult = mysqli_query($db, $checkUserSql);
				$checkUserResult = mysqli_num_rows($checkResult);
		
				if ($checkUserResult == 0){
					$sql = "INSERT INTO user VALUES ('$name', '$uname', '$pwd')";
					$query = mysqli_query($db, $sql);
					$sql_check = "SELECT name, uname, password FROM user WHERE uname='$uname' AND name='$name'";
